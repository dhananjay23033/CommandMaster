...
void LS::executeR(bool hasL, std::filesystem::path path) {
    ...
    // Iterate over entries in the directory and launch tasks asynchronously
    for (const auto& entry : std::filesystem::recursive_directory_iterator(path)) {
        // Launch a task asynchronously for each entry
        futures.emplace_back(std::async(std::launch::async, &LS::print, this, hasL, entry.path()));

        // Check the number of tasks created and limit based on available cores
        if (futures.size() >= numCores) {
            // Wait for the tasks to complete before launching more
